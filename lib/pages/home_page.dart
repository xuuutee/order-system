...
class _HomePageState extends State<HomePage> {
  ...
  void initState() {
    ...
    _listenNewOrders();
    // 每 5 分钟后台检查新版本
    _versionTimer = Timer.periodic(const Duration(minutes: 5), (_) => _checkUpdate());
  }
  ...
  Future<void> _checkUpdate() async {
    ...
    if (!mounted) return;
    if (!info.hasUpdate || info.downloadUrl.isEmpty) return;
    // 避免重复弹同一个版本
    if (info.latestVersion == _lastNotifiedVersion) return;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      bottomNavigationBar: NavigationBar(
        ...
        destinations: const [
          NavigationDestination(icon: Icon(Icons.list_alt), label: '订单列表'),
          NavigationDestination(icon: Icon(Icons.add_circle_outline, size: 28), label: '新建订单'),
          NavigationDestination(icon: Icon(Icons.bar_chart), label: '统计总览'),
